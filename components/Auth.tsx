      ...
      case 1:
        return <>
          ...
          <div className={s.auth__form__container__input}>
            ...
            {+delay <= 30 ? <p>{locale === 'ru' ? 'Отправить код ещё раз через' : 'Send the code again in'} {delay} {locale === 'ru' ? 'секунд' : 'seconds'}</p> : <p onClick={sendCode}>{locale === 'ru' ? 'Отправить код ещё раз' : 'Send code again'}</p>}
          </div>
        </>
    }
  }
